...
public class FinalBitmap {
	...
	private static FinalBitmap imageManager;
	 ...
	 * @return
	 */
	public static FinalBitmap from(Context context) {
		...
		if (imageManager == null) {
			imageManager = new FinalBitmap(context);
		}
	...
	public void display(ImageView imageView, String url, Bitmap bm){
		if (imageView == null) {
			return;
		}
		...
		queueImage(new ImageRef(imageView, url, filePath, 0));
	}
	/**
	 * 显示图片
	 * 
...
//		}
	...
	private byte[] loadByteArrayFromNetwork(String url) {
		...
		try {
			...
			HttpResponse response = NineXiuClient.getInstance().getHttpClient().execute(method);
