...
public class Panel extends LinearLayout {
	...
	public Panel(Context context, AttributeSet attrs) {
		...
		mDuration = a.getInteger(R.styleable.Panel_animationDuration, 550); // duration
																			// defaults
																			// to
																			// 750
																			// ms
...
//		mHandle.setOnClickListener(clickListener);
	...
	OnClickListener clickListener = new OnClickListener() {
		...
		public void onClick(View v) {
			if (isOpen()) {
				mGestureListener.setScroll(0, 0);
				if(mMotionEvent!=null)
				mMotionEvent.offsetLocation(0, 0);
				post(startAnimation);
			}
		}
	};
		...
		public void run() {
			...
			if (mOrientation == VERTICAL) {
				...
				if (mState == State.FLYING && mLinearFlying) {
					calculatedDuration = (int) (500 * Math
