 ...
 * accessed, it is moved to the head of a queue. When a value is added to a full cache, the value
 * at the end of that queue is evicted and may become eligible for garbage collection.
 * 
 * <p>
 ...
 * {@link #create}. This simplifies the calling code, allowing it to assume a value will always be
 * returned, even when there's a cache miss.
 * 
 * <p>
 * By default, the cache size is measured in the number of entries. Override {@link #sizeOf} to
 * size the cache in different units. For example, this cache is limited to 4MiB of bitmaps:
 * 
 * <pre>
 * {@code
 ...
 * 
 * <p>
 * This class is thread-safe. Perform multiple cache operations atomically by synchronizing on the
 * cache:
 * 
 * <pre>
 * {@code
 ...
 * {@link #get}, {@link #put} or {@link #remove} is unambiguous: the key was not in the cache.
 */
public class LruCache<K, V> {
	...
	public final V get(K key) {
		 ...
		 * create() returns. If a conflicting value was added to the map while create() was
		 * working, we leave that value in the map and release the created value.
		 */
	 ...
	 * 
	 * @param maxSize
	 *            the maximum size of the cache before returning. May be -1 to evict even 0-sized
	 *            elements.
	 */
	public void trimToSize(int maxSize) {
		while (true) {
			...
			synchronized (this) {
				...
				size -= safeSizeOf(key, value);
				evictionCount++;
				VolleyLog.d("Evict the eldest entry [%s], remaining size [%d], maxSize [%d]", key,
						size, maxSize);
			}
	 ...
	 * 
	 * @param evicted
	 *            true if the entry is being removed to make space, false if the removal was caused
	 *            by a {@link #put} or {@link #remove}.
	 * @param newValue
	 *            the new value for {@code key}, if it exists. If non-null, this removal was caused
	 *            by a {@link #put}. Otherwise it was caused by an eviction or a {@link #remove}.
	 */
