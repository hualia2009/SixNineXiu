...
public class VipActivity extends Activity implements OnClickListener{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		vipListView=(PullListView) findViewById(R.id.lv_vip);
		vipCategory=(LinearLayout) findViewById(R.id.vip_top);
		mUser = ApplicationEx.get().getUserManager().getUser();
		initView();
		parseData();
	}
	private void initView(){
		one_text = (TextView) findViewById(R.id.vip_one_text);
		three_text = (TextView) findViewById(R.id.vip_three_text);
		nine_text = (TextView) findViewById(R.id.vip_nine_text);
		twelve_text = (TextView) findViewById(R.id.vip_twelve_text);
		priceTextViews = new TextView[]{one_text,three_text,nine_text,twelve_text};
		one_img = (ImageView) findViewById(R.id.vip_one_img);
		three_img = (ImageView) findViewById(R.id.vip_three_img);
		nine_img = (ImageView) findViewById(R.id.vip_nine_img);
		tweleve_img = (ImageView) findViewById(R.id.vip_twelve_img);
			...
			try {
				...
				if(success==true){
					...
					for (  int i = 0; i < data.size(); i++) {
						...
						params.weight=1;
						TextView tv=(TextView) LayoutInflater.from(VipActivity.this).inflate(R.layout.shop_text_view_layout, null);
						if(i==0){
...
//								VipAdapter adapter=new VipAdapter(VipActivity.this, bean,false,null);
//								vipListView.setAdapter(adapter);
								VipActivity.this.bean =bean;
								for (int i = 0; i < 4; i++) {
...
//			buyVipTask(bean, 12+"");
	...
	private void buyVipTask(VipBean vipBean,String month) {
		...
		asyncHttpClient.post(url, params, new AsyncHttpResponseHandler(){
			...
			public void onStart() {
				super.onStart();
				mDialog=Utils.showProgressDialog(VipActivity.this, "购买中...");
				mDialog.show();
			}
			@Override
			public void onSuccess(String content) {
				...
				if(content!=null){
					...
					try {
						...
						boolean success=jsonObject.optBoolean("success");
						if(success==true){
							Utils.MakeToast(VipActivity.this, "购买成功");
							Utils.UpdateUserInfo(mUser, VipActivity.this);
						}else{
							String msg=jsonObject.optString("msg");
							Utils.MakeToast(VipActivity.this, msg);
						}
					} catch (Exception e) {
						e.printStackTrace();
					}
			...
			public void onFailure(Throwable error, String content) {
				...
				mDialog.dismiss();
				Utils.MakeToast(VipActivity.this, "网络连接超时");
