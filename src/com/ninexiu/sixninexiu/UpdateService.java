...
public class UpdateService extends Service {
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		try {
			...
			if (file.exists()) {
				...
				resultData.putString("update_file_path", FileUtil.getUpdateFilePath(fileNameString));
		...
		FileUtil.createFile(app_name + bundle.getString("version_name"));
	...
	public void createNotification() {
		...
		try {
			notification = new Notification(R.drawable.logo,// 应用的图标
	...
	public long downloadUpdateFile(String down_url, String file) throws Exception {
		...
		while ((readsize = inputStream.read(buffer)) != -1) {
			...
			if (updateCount == 0 || (downloadCount * 100 / totalSize - down_step) >= updateCount) {
				...
				Bundle resultData = new Bundle();
				resultData.putInt("progress", updateCount);
				receiver.send(UPDATE_PROGRESS, resultData);
			}
		}
